...
- цветное изображение — `palmer.png`, `1110 x 740`, формат `PNG`;
...
- `G_norm = 35` -> `43192` контурных пикселя (`5.26%`);
- `G_norm = 40` -> `34399` (`4.19%`);
- `G_norm = 45` -> `28140` (`3.43%`).
...
- `palmer`: `G_norm = 253`, то есть `T = 714.64`, итог `4` контурных пикселя (`0.00%`);
- `zhest`: `G_norm = 46`, то есть `T = 125.90`, итог `32071` контурный пиксель (`15.04%`).

Сравнение с текущими выбранными порогами:

- для `palmer` критерий Отсу даёт практически вырожденный порог, очень близкий к максимуму градиента: остаются только `4` контурных пикселя, число компонент падает с `3245` до `3`, а крупнейшая компонента уменьшается с `8165` до `2`, то есть полезные контуры фактически исчезают;
...
1. `|∇f|_max / 12` — `palmer`: `96076` пикселей (`11.70%`), `zhest`: `48971` пикселей (`22.97%`). Карта слишком плотная, остаётся много лишнего отклика.
2. Отсу — `palmer`: `4` пикселя (`0.00%`), `zhest`: `32071` пиксель (`15.04%`). На одном изображении порог практически убирает все контуры, на другом оказывается слишком низким.
3. Экспериментальный — `palmer`: `34399` пикселей (`4.19%`), `zhest`: `29852` пикселя (`14.00%`). Лучше сохраняет основные контуры без чрезмерного фона.
...
**Цветное изображение** (`palmer.png`, `1110 x 740`):

- `Gx`: `[-719.97; 709.33]`, `Gy`: `[-682.39; 699.02]`, `|∇f|`: `[0.00; 720.29]`
- уровень на `G_norm`: `40`, порог `T = 112.99`
- контурных пикселей: `34399` (`4.19%`)
...
Бинаризованное контурное изображение при `G_norm = 40`, что соответствует `T = 112.99`:
